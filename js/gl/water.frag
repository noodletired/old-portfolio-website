...
#define waterMaxDepth 6.0   // controls depth-colour blending
#define waveDensity 15.0    // controls amount of surfave waves
#define surfaceCutoff 0.88  // controls size & quantity of surface waves
#define foamMaxDepth 0.5    // controls foam max thickness
#define foamMinEdge 0.15    // controls minimum foam edge thickness
#define distortAmount 0.1   // controls noise distortion, slightly affects size of waves
#define AA 0.01             // antialiasing for smoothstep
...
void main(){
  ...
  float sceneDepth = perspectiveDepthToViewZ( getBufferClipPosition( uDepthMap ).z, cameraNear, cameraFar );
  float waterDepth = perspectiveDepthToViewZ( vWaterClipPosition.z, cameraNear, cameraFar );
  float depthDifference = waterDepth - sceneDepth;
